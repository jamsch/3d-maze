function Location3D(config) {
	...
	this.physical = config.physical; //anything but start location is true
	this._ghosted = false;
	...
	if (config.objPath != undefined && config.matPath != undefined) {
		//console.log("Loading " +config.objPath);
...
Location3D.prototype.update = function(delta){
	if (this.hasObject() && this.isLoaded()) {
...
/**
 * Checks if character is near the location object
 * @return {boolean}
 */
